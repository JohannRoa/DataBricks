...
-- MAGIC %python
-- MAGIC # TODO
-- MAGIC (spark.read.table("events").withColumnRenamed("user_id","user").groupBy("user").pivot("event_name").count()
...
-- MAGIC # TODO
-- MAGIC (spark.read
-- MAGIC     .table("events_pivot").join(spark.read.table("transactions"),col("user")==col("user_id"),"inner")
