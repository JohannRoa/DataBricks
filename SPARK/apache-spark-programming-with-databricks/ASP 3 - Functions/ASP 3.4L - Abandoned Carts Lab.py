...
# TODO
conversions_df = (users_df.join(converted_users_df,"email","outer").filter(col("email").isNotNull()).na.fill({"converted":False})
                 )
...
# TODO
abandoned_carts_df = (email_carts_df.filter("converted ==False").na.drop(subset="cart")
)
