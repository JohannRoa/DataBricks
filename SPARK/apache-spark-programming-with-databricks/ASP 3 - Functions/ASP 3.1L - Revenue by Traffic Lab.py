...
# TODO
final_df = (top_traffic_df.withColumns({"avg_rev":(top_traffic_df["avg_rev"]*100).cast("long")/100,"total_rev":(top_traffic_df["total_rev"]*100).cast("long")/100})
)
...
# TODO
bonus_df = (top_traffic_df.withColumns({"avg_rev":round(top_traffic_df["avg_rev"],2),"total_rev":round(top_traffic_df["total_rev"],2)})
)
...
# TODO
from pyspark.sql.functions import col
chain_df = (df.groupBy("traffic_source").agg(sum("revenue").alias("total_rev"),avg("revenue").alias("avg_rev")).sort("total_rev",ascending=False).limit(3).withColumns({"avg_rev":(col("avg_rev")*100).cast("long")/100,"total_rev":(col("total_rev")*100).cast("long")/100})
)
