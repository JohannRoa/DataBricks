...
df=spark.read.csv(source_file,sep=':',header=True).withColumns({"firstName": initcap(lower((col("firstName")))), "middleName": initcap(lower((col("middleName")))), "lastName": initcap(lower((col("lastName")))),"ssn":regexp_replace(col("ssn"),"-","")}).dropDuplicates()
